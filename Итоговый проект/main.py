...
class MainFrame(tk.Frame):
    ...
    #Создание основного интерфейса программы и тулбара
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'name', 'phone_number', 'email'), height=45, show='headings' )
    ...
    #Метод внесения данных в базу данных(добавления нового контакта)
    def records(self, name, phone_number, email):
        self.db.insert_data(name, phone_number, email)
        self.view_records()
    ...
    def update_records(self, name, phone_number, email):
        self.db.cursor.execute(
            '''UPDATE db SET name = ?, phone_number = ?, email = ? WHERE ID = ?''', (name, phone_number, email, self.tree.set(self.tree.selection() [0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
#Создание дочернего окна и его интерфейса
...
#Создание окна редактирования контакта
        ...
        try:
            ...
            btn_edit.bind(
                ...
                lambda event: self.view.update_records(
                    self.entry_name.get(), self.entry_email.get(), self.entry_phone_number.get()
                ),
            )
            ...
            messagebox.showerror("Error", "Пожалуйста, выберите контакт для редактирования.")
            self.destroy()
...
class DB:
    ...
    def insert_data(self, name, phone_number, email):
        self.cursor.execute('''INSERT INTO db (name, phone_number, email) VALUES(?,?,?)''', (name, phone_number, email))
        self.conn.commit()
